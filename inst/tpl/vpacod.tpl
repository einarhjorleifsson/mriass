...
//  init_int firstagewithconstantF; // Above this F is independed of age
  ...
  init_vector surveywts(1,nsurveys);
...
// limiting.  Have to read from *global_datafile  Three datafiles for each survey.  
  ...
  !!for (i = 1; i<=nsurveys; i++) {
   ...
   !! maxsurveylastage = max(surveylastage);
...
// Estimated variables;
 ...
 vector lnMigrationAbundance(1,MigrationNumbers);
 init_bounded_vector logMigrationRatios(1,MigrationNumbers,-3,0,5);
 matrix MigrationRatios(firstyear,lastoptyear,firstage,lastage);  // Proportion that migrated
	
 number lnMeanRecr;
 vector lnRecr(firstyear,lastoptyear+firstage-recrdatadelay) ;// log of recruitment
 number lnMeanInitialpop;
 vector lnInitialpop(firstage+1,lastage);
 vector  InitialSelection(firstcatchage,lastage-3);
// 0.2 to allow only 20  over.  
 matrix EstimatedSelection(firstcatchage,lastage-4,1,number_of_seperable_periods); // Same selction of last 3 age groups
 number Catchlogitslope;
 number Catchlogitage50;
 number logSigmaCmultiplier;
 init_bounded_vector Surveycorr(1,nsurveys,0.2,0.85,4);
//   number Surveycorr;
  number AbundanceMultiplier;
...
//  vector SigmaSurveypar(1,nsurveys);
  init_bounded_matrix SurveylnQest(1,nsurveys,minsurveyfirstage,maxsurveyfirstagewithfullcatchability,-40,-3,2);
  init_bounded_number Q1from2002(-2,2,3); // changed catchability
  init_bounded_matrix SigmaSurveypar(1,nsurveys,minsurveyfirstage,maxsurveylastage,-5,3,5);
...
//  init_bounded_matrix Fpar(firstyear,lastoptyear,1,Nbasfunc,-10,1.5,3);
  ...
  init_bounded_vector logSurvivors(firstage,lastage,1,20,2);  
...
// Parameters in SSB-recruitment.  
  ...
  !!dvector srub(1,6);
  !!ivector srphase(1,6);
  !!srlb(1)=4.0 ; srlb(2)=2.0 ; srlb(3)=-1.3 ; srlb(4)=-4.6 ; srlb(5)=-1.0 ; srlb(6)=-0.4;  // -0.2
  !!srub(1)=18.0 ; srub(2)=10.0 ; srub(3)=1.0 ; srub(4)=-0.1 ; srub(5)=1.0 ; srub(6)=0.4;//0.2
...
// sdreport vectors that need to be set.  
  sdreport_vector PredRefF(lastoptyear-5,lastyear); //sd
  sdreport_vector PredSpawningstock(lastoptyear-5,lastyear); //sd
  sdreport_vector PredN(lastoptyear-5,lastyear); //sd
  sdreport_vector Survivors(firstage,lastage); //sd

  vector RefF(firstyear,lastyear);
  vector Spawningstock(firstyear,lastyear);
  vector RelSpawningstock(firstyear,lastyear);
...
// Parameters that are sometimes estimated.  
  ...
  for(i = 1; i <= nsurveys; i++) SigmaSurveypar(i) = log(0.5);
  SurveylnQ = -100;
...
// Stop trend
 ...
 for(i = 1; i <= MigrationNumbers ; i++)
    if(int(MigrationYears(i)) >= firstyear-1  & int(MigrationYears(i)) <= lastoptyear)
	MigrationRatios(int(MigrationYears(i)),int(MigrationAges(i))) = mfexp(logMigrationRatios(i));


  int HistoricalAssessment = 1;
  N = 0;
  for(age = firstage; age <= lastage; age++) 
    N(lastoptyear+1,age) = mfexp(logSurvivors(age));
  for(year = lastoptyear; year >=  firstyear; year-- ) {
    CalcNaturalMortality1(year);
    for(age = firstcatchage; age <= lastage - 1; age++) 
      N(year,age) =  (N(year+1,age+1)*exp(natM(year,age)/2)+ObsCatchInNumbers(year,age))*exp(natM(year,age)/2)*MigrationRatios(year,age);;
    ...
    Z(year,lastage) = -log(Nnew/Nold);
    F(year,lastage) = Z(year,lastage) - natM(year,lastage);
    N(year,lastage) = ObsCatchInNumbers(year,lastage) *Z(year,lastage)/F(year,lastage)/(1-mfexp(-Z(year,lastage)));
  }

  for(year = firstyear; year <= lastoptyear; year++) {
     for(age = firstage; age < lastage; age++) {
       Z(year,age) = -log(N(year+1,age+1)/N(year,age))*MigrationRatios(year,age);  
       F(year,age) = Z(year,age) - natM(year,age);
     }
     // migration ratios might have to go here.  
//     CalcCatchInNumbers(year)=elem_prod(elem_div(F(year),Z(year)),elem_prod((1.-mfexp(-Z(year))),elem_prod(N(year),MigrationRatios(year))));
...
FUNCTION void IceCodHarvestRule(int yr)
// Adapt to Fishing Years.  
   ...
   dvariable refbio = sum(elem_prod(N(yr)(refage,lastage),CatchWeights(yr)(refage,lastage)));     
   dvariable refcatch = HarvestRatio*mfexp(log(refbio)+AssessmentErr(yr));
   Catch = (CurrentTac +  refcatch)/2;  
...
// Almanaks ár 
FUNCTION void IceCodHarvestRule1(int yr)
// Adapt to Fishing Years.  
   //dvariable HarvestRatio = 0.2;
   dvariable MaxHarvestRatio = 0.6; 
   dvariable Catch;
   ...
   Catch = SmoothDamper(Catch,MaxHarvestRatio*refbio,mincatch);
   CurrentTac = Catch;  
   ProgF(yr) = FishmortFromCatch(Catch,N(yr),CatchWeights(yr),progsel,natM(yr));
...
FUNCTION void SetAssessmentErr()
  ...
  AssessmentErr(lastoptyear+1) = AssessmentErr(lastoptyear+1)/ratio;
  ...
  AssessmentErr(lastoptyear+1)*=0.75;  // Partly included in Cv in numbers
//  AssessmentErr(lastoptyear+2)*=0.7;
...
// follow catch well. 
  ...
  LnLikelicomp(2) = SSB_Recruitment_loglikeliNocorr(); 

  for(i = 1; i <= nsurveys; i++) {
      Surveylikelihood(i) = Survey_loglikeli1(i); // Store by survey
      LnLikelicomp(3) +=  Surveylikelihood(i)*surveywts(i);
  }
...
// years in between but gives good enough results as initial guess.  
  ...
  for( i = surveyfirstyear(surveynr); i <= finalyear; i++) {
    for(j = surveyfirstage(surveynr); j <= surveylastage(surveynr); j++) {
      ...
      if(surveynr==1 && j==1 && i > 2002) // Breyting í Q eins árs
       CalcSurveyNr(surveynr,i,j) = mfexp(log(N(i,j)*mfexp(-pZ))*SurveyPower(surveynr,j)+SurveylnQ(surveynr,j)+Q1from2002+SurveylnYeareffect(surveynr,i));
      else        
...
FUNCTION  dvariable Survey_loglikeli1(int surveynr)
  ...
  for(i = surveyfirstage(surveynr); i <= surveylastage(surveynr); i++) 
    SigmaSurvey(surveynr,i) = mfexp(SigmaSurveypar(surveynr,i));
...
FUNCTION void ReadSurveyInfo(adstring parameterfilename,adstring datafilename, adstring residualfilename,int surveynumber,ofstream& surveylogfile)
  ...
  surveylogfile <<  SigmaSurveyInp(surveynumber) << endl; 
  ...
  cout << SurveyResolution(surveynumber);
  surveylogfile << endl;
...
FUNCTION void ReadPrognosis()
   ofstream outfile("prognosis.log");
    ...
    if(CatchRule == 1  || CatchRule == 2) { // F or Catch read next ncatch years, fill the rest with last value
        infile >> nCatchorFyrs;
